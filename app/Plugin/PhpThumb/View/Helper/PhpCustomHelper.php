...
class PhpCustomHelper extends HtmlHelper {
    ...
    private function set_cache_filename()    {
       ...
       $newFileName =  explode('/',$this->options['src']);
	    $new_File_count = count($newFileName);
	    //print_r($newFileName);exit;
        $this->cache_filename = $this->options['save_path'] . DS . $newFileName[$new_File_count-1];
    ...
    function generateThumbnail($image, $options) {
        $pathToDefine = explode('/',$image);
		$thumbs_path = $pathToDefine[0] .'/medium/';
		echo $thumbs_path;
