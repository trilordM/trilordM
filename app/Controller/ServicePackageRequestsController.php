 ...
 */
	public function add($slug=null) {
		...
		$servicePackages = $this->ServicePackageRequest->ServicePackage->find('first',array('conditions'=>array('ServicePackage.slug'=>$slug,'ServicePackage.is_active'=>1),'recursive'=>-1));
		if ($this->request->is('post')) {
			...
			{
				$this->Session->setFlash('You do not have enough balance to process the request. Please recharge your account.','default',array('class'=>'error-message'));
			}else
			{
				$this->ServicePackageRequest->create();
				if ($this->ServicePackageRequest->save($this->request->data)) {
					$this->Session->setFlash('The service package request has been processed.','default',array('class'=>'success'));
					return $this->redirect(array('action' => 'add',$slug));
				} else {
					$this->Session->setFlash('The service package request could not be saved. Please, try again.','default',array('class'=>'error-message'));
				}
			}
		}
	...
	public function admin_lock($id = null, $status = null) {
		...
		if($status==0){
			...
			if ($this->ServicePackageRequest->save($this->request->data)) {
				...
				$this->Session->setFlash('The request could not be locked. Please, try again.','default',array('class'=>'error-message'));
			}
			...
			if ($this->ServicePackageRequest->save($this->request->data)) {
				...
				$this->Session->setFlash('The request could not be unlocked. Please, try again.','default',array('class'=>'error-message'));
			}
		}
 ...
 */
	public function admin_add() {
		if ($this->request->is('post')) {
			$this->ServicePackageRequest->create();
			if ($this->ServicePackageRequest->save($this->request->data)) {
				$this->Session->setFlash('The service package request has been saved.','default',array('class'=>'success'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash('The service package request could not be saved. Please, try again.','default',array('class'=>'error-message'));
			}
 ...
 */
	public function admin_assign($id=null) {
		...
		$servicePackages = $this->ServicePackageRequest->find('first',array('conditions'=>array('ServicePackageRequest.id'=>$id)));
		if ($this->request->is('post')) {
			$this->request->data['ServicePackageRequest']['id'] = $id;
			...
			if ($this->ServicePackageRequest->save($this->request->data)) {
				...
				//debug($servicePackages);exit;				
							$this->ServicePackageRequest->query("
											...
											");
				endforeach;
				$this->Session->setFlash('The service package request has been assigned.','default',array('class'=>'success'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash('The service package request could not be saved. Please, try again.','default',array('class'=>'error-message'));
			}
 ...
 */
	public function admin_edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->ServicePackageRequest->save($this->request->data)) {
				$this->Session->setFlash('The service package request has been updated.','default',array('class'=>'success'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash('The service package request could not be updated. Please, try again.','default',array('class'=>'error-message'));
			}
		} else {
			$options = array('conditions' => array('ServicePackageRequest.' . $this->ServicePackageRequest->primaryKey => $id));
			$this->request->data = $this->ServicePackageRequest->find('first', $options);
 ...
 */
	public function admin_delete($id = null) {
		...
		$this->request->onlyAllow('post', 'delete');
		if ($this->ServicePackageRequest->delete()) {
			$this->Session->setFlash('The service package request has been deleted.','default',array('class'=>'success'));
		} else {
			$this->Session->setFlash('The service package request could not be deleted. Please, try again.','default',array('class'=>'error-message'));
		}
		return $this->redirect(array('action' => 'index'));
	}
