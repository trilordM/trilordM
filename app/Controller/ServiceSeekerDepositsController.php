 ...
 */
	public function index() {
		...
		if(!empty($this->params->query)){
			...
			$transactionStatus = '';
			$this->Paginator->settings = array(
										'conditions' => array('ServiceSeekerDeposit.status' => 'New Deposit', 'ServiceSeekerDeposit.user_id' => $this->Auth->user('id')),
										'order' => 'ServiceSeekerDeposit.deposited_date'
									);
		}
	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->ServiceSeekerDeposit->save($this->request->data)) {
				$this->Session->setFlash('The service seeker deposit has been saved.','default',array('class'=>'success'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash('The service seeker deposit could not be saved. Please, try again.','default',array('class'=>'error-message'));
			}
		}
 ...
 */
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->ServiceSeekerDeposit->save($this->request->data)) {
				$this->Session->setFlash('The service seeker deposit has been updated.','default',array('class'=>'success'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash('The service seeker deposit could not be updated. Please, try again.','default',array('class'=>'error-message'));
			}
		} else {
			$options = array('conditions' => array('ServiceSeekerDeposit.' . $this->ServiceSeekerDeposit->primaryKey => $id));
 ...
 */
	public function delete($id = null) {
		...
		if ($this->ServiceSeekerDeposit->delete()) {
			$this->Session->setFlash('The service seeker deposit has been deleted.','default',array('class'=>'success'));
		} else {
			$this->Session->setFlash('The service seeker deposit could not be deleted. Please, try again.','default',array('class'=>'error-message'));
		}
		return $this->redirect(array('action' => 'index'));
	}
 ...
 */
	public function admin_verify($id = null){
		...
		if ($this->ServiceSeekerDeposit->save($this->request->data)) {
			...
			$this->Session->setFlash('The deposit could not be verified. Please, try again.','default',array('class'=>'success'));
		}
		return $this->redirect(array('action' => 'index'));
	...
	public function admin_index() {
		...
		if(!empty($this->params->query)){
			...
			if ($type=='export') {
				...
				$serviceSeekerDeposits=$this->ServiceSeekerDeposit->find('all',array('conditions'=>array($conditions)));
 ...
 */
	public function admin_edit($id = null) {
		...
		$users = $this->ServiceSeekerDeposit->User->find('list',array('conditions'=>array('User.role'=>'ServiceSeeker','User.status'=>1)));
