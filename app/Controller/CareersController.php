	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->Career->save($this->request->data)) {
				...
				$this->Session->setFlash('The career could not be saved. Please, try again.','default',array('class'=>'error-message'));
			}
		}
	}
	...
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Career->save($this->request->data)) {
				...
				$this->Session->setFlash('The career could not be updated. Please, try again.','default',array('class'=>'error-message'));
			}
		} else {
			$options = array('conditions' => array('Career.' . $this->Career->primaryKey => $id));
 ...
 */
	public function delete($id = null) {
		...
		if ($this->Career->delete()) {
			...
			$this->Session->setFlash('The career could not be deleted. Please, try again.','default',array('class'=>'error-message'));
		}
		return $this->redirect(array('action' => 'index'));
	}
...
public function career() {
	$vacancy=$this->Career->find('all',array('conditions'=>array('is_active'=>'1','valid_till >='=>date('Y-m-d'))));
	//debug($vacancy);die;
    $hideSearchBar = true;
