	...
	public function add() {
		...
		if ($this->request->is('post')) {
			...
			if ($this->Testimonial->save($this->request->data)) {
				...
				$this->Session->setFlash('The testimonial could not be saved. Please, try again.','default',array('class'=>'error-message'));
			}
		}
	...
	public function admin_verify($id = null,$status=null){
		...
		if($status=='enable'){
			...
			if ($this->Testimonial->save($this->request->data)) {
						...
						$this->Session->setFlash('Unable to disable testimonial.','default',array('class'=>'error-message'));
					}
		}
		return $this->redirect(array('action' => 'index'));
	}
	...
	public function admin_edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Testimonial->save($this->request->data)) {
				...
				$this->Session->setFlash('The testimonial could not be updated. Please, try again.','default',array('class'=>'error-message'));
			}
		} else {
			$options = array('conditions' => array('Testimonial.' . $this->Testimonial->primaryKey => $id));
 ...
 */
	public function admin_delete($id = null) {
		...
		if ($this->Testimonial->delete()) {
			...
			$this->Session->setFlash('The testimonial could not be deleted. Please, try again.','default',array('class'=>'error-message'));
		}
		return $this->redirect(array('action' => 'index'));
	}}
