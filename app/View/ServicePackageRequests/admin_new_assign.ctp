...
<?php if(!empty($Assigned_Provider)){?>
<div>
<?php echo __("List of already assigned Provider's");?>
<ul>
<?php //debug($Assigned_Provider);die;
foreach($Assigned_Provider as $provider):?>
<li>
<?php echo $this->SmartForm->getUserInfo($provider['service_package_assigned_providers']['provider_id']); ?>
</li>
<?php endforeach; ?>
</ul>
</div>
